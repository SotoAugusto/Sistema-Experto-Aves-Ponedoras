"""
 ...
 La libreria 'experta'  está hecha para hacer sistemas expertos, 
 está inspirado en el lenguaje CLIPS creado por la NASA.
...
"""
    ...
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(nombre=W())), 
          salience=1000)
    ...
    """
    Inicio de la obteicón de sintomas, se utiliza una REGLA (RULE)
    que funciona como método para establecer qué sintomas 
    tiene el ave como HECHO (fact), se le asigna una
    prioridad o importancia al sintoma (salience) la cual será 
    utilizada posteriormente en caso de no haber un match 
    exacto conalguna enfermedad e inferir la enfermedad 
    más probable. Se solicita el input del usuario,
    el input se pasa a minusculas y elimina whitespace para
    mejor manejo de los datos.
    """
    # Encontrar sintoma secrecion_nasal
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(secrecion_nasal=W())), 
          salience=995)
    def tienesecrecion_nasal(self):
        ...
        self.declare(
            Fact(secrecion_nasal=self.secrecion_nasal.strip().lower()))

    # Encontrar sintoma secrecion_ocular
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(secrecion_ocular=W())), 
          salience=985)
    def tienesecrecion_ocular(self):
        ...
        self.declare(
            Fact(secrecion_ocular=self.secrecion_ocular.strip().lower()))

    # Encontrar sintoma jadeo
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(jadeo=W())), 
    salience=975)
    def tienejadeo(self):
        self.jadeo = input(
            "\nTiene jadeo?\nPor favor introduzca si/no\n")
    ...
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(espasmos=W())), 
    salience=970)
    ...
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(rigidez=W())), 
    salience=965)
    ...
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(tos=W())), 
    salience=955)
    def tienetos(self):
        self.tos = input(
            "\nTiene tos?\nPor favor introduzca si/no\n")
    ...
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(estornudos=W())), 
    salience=950)
    ...
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(plumas_erizadas=W())), 
    salience=945)
    ...
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(fiebre=W())), 
    salience=940)
    ...
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(diarrea=W())), 
    salience=935)
    ...
    @Rule(
        Fact(findEnfermedad="true"), NOT(Fact(infeccion_bolsa_fabricio=W())), 
        salience=930
    )
    ...
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(contracciones_cuello=W())), 
    salience=925)
    ...
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(falta_apetito=W())), 
    salience=920)
    ...
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(inflamacion_cara=W())), 
    salience=915)
    def tieneinflamacion_cara(self):
        ...
        self.declare(
            Fact(inflamacion_cara=self.inflamacion_cara.strip().lower()))

    # Encontrar sintoma sensibilidad luz
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(sensibilidad_luz=W())), 
    salience=910)
    def tienesensibilidad_luz(self):
        ...
        self.declare(
            Fact(sensibilidad_luz=self.sensibilidad_luz.strip().lower()))
    ...
    """ 
        ...
        Sintomas: 
        secrecion_nasal,plumas_erizadas,fiebre,
        diarrea,contracciones_cuello,falta_apetito
    """
    @Rule(
        Fact(findEnfermedad="true"),
        Fact(secrecion_nasal="si"),
    ...
    # la importancia de los sintomas encontrados (salience)
    # (en caso de haber sintomas)
    @Rule(Fact(findEnfermedad="true"), NOT(Fact(Enfermedad=W())), 
          salience=-1)
    def unmatched(self):
        self.declare(Fact(Enfermedad="unknown"))

    @Rule(Fact(findEnfermedad="true"), Fact(Enfermedad=MATCH.Enfermedad), 
          salience=1)
    def getEnfermedad(self, Enfermedad):
        ...
        if Enfermedad == "unknown":
            ...
            if max_val == 0:
                print(
                    "No se han encontrado enfermedades. El ave está sana.")
            else:
...
"\n\nNo podemos decirle con certeza cuál es la enfermedad exacta, pero creemos que padece...",
 ...
 # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #                     
                ...
                f = open("disease/disease_descriptions/" + pred_enf + 
                         ".txt", "r")
                print(f.read())
                print('''
 # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #                     
                ...
                f = open("disease/disease_treatments/" + pred_enf + 
                         ".txt", "r")
                print(f.read())
                print('''
 # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #                     
            ...
            print(
                "La enfermedad más probable que padezca es:", Enfermedad)
            print("\n\n")
            print('''
 # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #                     
                      ''')
            ...
            # abre el archivo txt correpondiente a la enfermedad
            # donde obtendrá su descripción
            f = open("disease/disease_descriptions/" + Enfermedad + 
                     ".txt", "r")
            print(f.read())
            print('''
 # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #                     
            ...
            # abre el archivo txt correpondiente a la enfermedad
            # donde obtendrá su tratamiento
            f = open("disease/disease_treatments/" + Enfermedad + 
                     ".txt", "r")
            print(f.read())
